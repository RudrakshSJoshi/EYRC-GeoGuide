...
def detect_ArUco_details(image):
    ...
    First output: The dictionary `ArUco_details_dict` should have the id of the marker
    ...
    * ArUco_corners =
    ...
    if ids is not None:
        for i in range(len(ids)):
            marker_id = int(ids[i][0])  # Convert the ID to int
            corner_points = corners[i][0].astype(float)

            # Calculate the center
            center_x = int(np.mean(corner_points[:, 0]))
            center_y = int(np.mean(corner_points[:, 1]))

            # Calculate the angle from the vertical (assuming that the marker is upright)
            angle_rad = math.atan2(corner_points[2][1] - corner_points[0][1], corner_points[2][0] - corner_points[0][0])
            angle = int(math.degrees(angle_rad)) # Ensure the angle is an int

            # Fill the dictionaries
            ArUco_details_dict[marker_id] = [[center_x, center_y], angle-45]
...
if __name__ == "__main__":
    ...
    img_dir_path ="D:\\HP\\users\\OneDrive\\Desktop\\eyantra\\Task_1A\\GG_3047_Task2A\\public_test_cases"
